...
void Rover::update_wheel_encoder()
{
    ...
    if (sensor_diff_ms == 0 || sensor_diff_ms > 100) {
        // No new reading since we last sent this wheel to the EKF. For a
        // locally decoded encoder that is itself a measurement - no pulse
        // means the wheel is not turning - so fall back to wall clock and
        // send the (zero) rotation. For a sensor fed by an external stream
        // it only means no data arrived, and sending zero would tell the
        // EKF the wheel had stopped when we simply do not know, so skip.
        if (!g2.wheel_encoder.no_data_means_stopped(wheel_encoder_last_index_sent)) {
            // re-baseline before giving up on this reading, otherwise a gap
            // in the stream would leave sensor_diff_ms permanently over the
            // limit and this wheel would never be sent again
